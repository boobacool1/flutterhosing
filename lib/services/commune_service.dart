...
import 'package:real_estate_agency/modeles/t_commune.dart';
...
class CommuneService {
  ...
  Future<List<TCommune>> getCommunes() async {
    Response res = await get('$apiUrl/communes');

    if (res.statusCode == 200) {
      String source = Utf8Decoder().convert(res.bodyBytes);
      List<dynamic> body = jsonDecode(source);

      List<TCommune> communes =
          body.map((dynamic item) => TCommune.fromJson(item)).toList();
      return communes;
    } else {
